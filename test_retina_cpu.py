from __future__ import print_function
import os
import argparse
import torch
import torch.backends.cudnn as cudnn
import numpy as np
import time
from data import cfg_mnet, cfg_re50
from layers.functions.prior_box import PriorBox
from utils.nms.py_cpu_nms import py_cpu_nms
import cv2
from models.retinaface import RetinaFace
from utils.box_utils import decode, decode_landm

parser = argparse.ArgumentParser(description='Retinaface CPU Test')
parser.add_argument('-m', '--trained_model', default='./weights/mobilenet0.25_Final.pth', type=str)
parser.add_argument('--network', default='mobile0.25', help='mobile0.25 or resnet50')
parser.add_argument('--dataset_folder', default='data/widerface/test/images2/', type=str)
parser.add_argument('--label_file', default='data/widerface/test/label2.txt', type=str)
parser.add_argument('--save_folder', default='result_test/result_test_cpu/', type=str)
parser.add_argument('--confidence_threshold', default=0.02, type=float)
parser.add_argument('--nms_threshold', default=0.4, type=float)
args = parser.parse_args()

def check_keys(model, pretrained_state_dict):
    ckpt_keys = set(pretrained_state_dict.keys())
    model_keys = set(model.state_dict().keys())
    used_pretrained_keys = model_keys & ckpt_keys
    assert len(used_pretrained_keys) > 0, 'load NONE from pretrained checkpoint'

def remove_prefix(state_dict, prefix):
    return {key[len(prefix):] if key.startswith(prefix) else key: val for key, val in state_dict.items()}

def load_model(model, pretrained_path):
    pretrained_dict = torch.load(pretrained_path, map_location='cpu')
    if "state_dict" in pretrained_dict.keys():
        pretrained_dict = remove_prefix(pretrained_dict['state_dict'], 'module.')
    else:
        pretrained_dict = remove_prefix(pretrained_dict, 'module.')
    check_keys(model, pretrained_dict)
    model.load_state_dict(pretrained_dict, strict=False)
    return model

if __name__ == '__main__':
    torch.set_grad_enabled(False)
    cfg = cfg_mnet if args.network == "mobile0.25" else cfg_re50

    net = RetinaFace(cfg=cfg, phase='test')
    net = load_model(net, args.trained_model)
    net.eval()
    cudnn.benchmark = False
    device = torch.device("cpu")
    net = net.to(device)

    # Load image paths
    with open(args.label_file, 'r') as f:
        image_list = [line.strip() for line in f if line.strip() and not line.startswith('#')]

    # Create output folders
    os.makedirs(os.path.join(args.save_folder, 'txt'), exist_ok=True)
    os.makedirs(os.path.join(args.save_folder, 'images'), exist_ok=True)

    t_start = time.time()

    for i, rel_path in enumerate(image_list):
        img_path = os.path.join(args.dataset_folder, rel_path)
        img_raw = cv2.imread(img_path, cv2.IMREAD_COLOR)
        if img_raw is None:
            continue

        img = np.float32(img_raw)
        im_height, im_width, _ = img.shape
        scale = torch.Tensor([im_width, im_height, im_width, im_height])
        img -= (104, 117, 123)
        img = img.transpose(2, 0, 1)
        img = torch.from_numpy(img).unsqueeze(0).to(device)
        scale = scale.to(device)

        loc, conf, landms = net(img)
        priorbox = PriorBox(cfg, image_size=(im_height, im_width))
        priors = priorbox.forward().to(device)
        prior_data = priors.data

        boxes = decode(loc.data.squeeze(0), prior_data, cfg['variance']) * scale
        boxes = boxes.cpu().numpy()
        scores = conf.squeeze(0).data.cpu().numpy()[:, 1]
        landms = decode_landm(landms.data.squeeze(0), prior_data, cfg['variance'])
        scale1 = torch.Tensor([img.shape[3], img.shape[2]] * 5).to(device)
        landms = (landms * scale1).cpu().numpy()

        inds = np.where(scores > args.confidence_threshold)[0]
        boxes = boxes[inds]
        landms = landms[inds]
        scores = scores[inds]
        order = scores.argsort()[::-1]
        boxes = boxes[order]
        landms = landms[order]
        scores = scores[order]
        dets = np.hstack((boxes, scores[:, np.newaxis])).astype(np.float32, copy=False)
        keep = py_cpu_nms(dets, args.nms_threshold)
        dets = dets[keep, :]
        landms = landms[keep]
        dets = np.concatenate((dets, landms), axis=1)

        save_txt = os.path.join(args.save_folder, 'txt', rel_path.replace('.jpg', '.txt'))
        os.makedirs(os.path.dirname(save_txt), exist_ok=True)
        with open(save_txt, 'w') as fd:
            fd.write(f"{rel_path}\n")
            fd.write(f"{len(dets)}\n")
            for b in dets:
                x, y, x2, y2, conf = b[:5]
                w, h = int(x2 - x), int(y2 - y)
                fd.write(f"{int(x)} {int(y)} {w} {h} {conf:.10f}\n")

        result_img_path = os.path.join(args.save_folder, 'images', rel_path)
        os.makedirs(os.path.dirname(result_img_path), exist_ok=True)
        for b in dets:
            if b[4] < args.confidence_threshold:
                continue
            b = list(map(int, b))
            cv2.rectangle(img_raw, (b[0], b[1]), (b[2], b[3]), (0, 255, 0), 2)
            for j in range(5):
                cv2.circle(img_raw, (b[5 + j*2], b[5 + j*2 + 1]), 1, (0, 0, 255), 4)
        cv2.imwrite(result_img_path, img_raw)

    t_end = time.time()
    with open(os.path.join(args.save_folder, 'time.txt'), 'w') as f:
        f.write(f"Start: {t_start}\nEnd: {t_end}\nTotal: {t_end - t_start:.2f} seconds\n")
